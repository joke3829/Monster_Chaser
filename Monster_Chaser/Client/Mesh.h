...
public:
	...
	Mesh(XMFLOAT3& center, XMFLOAT3& extent, std::string meshName = "noNameMesh");					// boundingOBB Create
...
private:
	...
	ComPtr<ID3D12Resource> m_pd3dTexCoord0Buffer{};		// DXGI_FORMAT_R32G32B32_UNORM;
	bool m_bHasTex0 = false;

	UINT m_nTexCoord1Count{};							// <TextureCoords1>
	ComPtr<ID3D12Resource> m_pd3dTexCoord1Buffer{};		// DXGI_FORMAT_R32G32B32_UNORM;
	...
	UINT m_nSubMeshesCount{};							// m_vSuvMeshes.size()
	std::vector<ComPtr<ID3D12Resource>> m_vSubMeshes;	// SubMesh(indexbuffer)
	std::vector<UINT> m_vIndices;						// index count vector
