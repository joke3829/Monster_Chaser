...
void CPlayerMage::ProcessInput(UCHAR* keyBuffer, float fElapsedTime)
{
	...
	// W + A + Shift -> Run Left Up
		...
		if (!(m_PrevKeyBuffer['W'] & 0x80) || !(m_PrevKeyBuffer['A'] & 0x80) || !(m_PrevKeyBuffer[VK_LSHIFT] & 0x80)) {
			m_AManager->ChangeAnimation(ANI_RUN_LEFT_UP, true); // Run Left Up
			m_Object->SetLookDirection(characterDir, XMFLOAT3(0.0f, 1.0f, 0.0f));
			m_AManager->UpdateAniPosition(0.0f, m_Object);
		}
		else {
			m_AManager->ChangeAnimation(ANI_RUN_LEFT_UP, true); // Maintain Run
		}
	}
	// W + A + Shift -> Walk Left Up
	else if ((keyBuffer['W'] & 0x80) && (keyBuffer['A'] & 0x80) && (m_PrevKeyBuffer[VK_LSHIFT] & 0x80) && !(keyBuffer[VK_LSHIFT] & 0x80)) {
		m_AManager->ChangeAnimation(ANI_WALK_LEFT_UP, true); // Walk Left UP
		m_Object->SetLookDirection(characterDir, XMFLOAT3(0.0f, 1.0f, 0.0f));
		m_AManager->UpdateAniPosition(0.0f, m_Object);
	}
	// W + A -> Walk Left Up
		...
		if (!(m_PrevKeyBuffer['W'] & 0x80) || !(m_PrevKeyBuffer['A'] & 0x80)) {
			...
			m_AManager->ChangeAnimation(ANI_WALK_LEFT_UP, true); // Maintain Walk
		}
	}
	// W + A + Shift, A -> Run Forward
	else if ((keyBuffer['W'] & 0x80) && (keyBuffer[VK_LSHIFT] & 0x80) && (m_PrevKeyBuffer['A'] & 0x80) && !(keyBuffer['A'] & 0x80)) {
		m_AManager->ChangeAnimation(ANI_RUN_FORWARD, true); // Run Forward
		m_Object->SetLookDirection(characterDir, XMFLOAT3(0.0f, 1.0f, 0.0f));
		m_AManager->UpdateAniPosition(0.0f, m_Object);
	}
	// W + A, A -> Walk Forward
	...
	// W + A, W -> Walk Left
		...
		m_AManager->ChangeAnimation(ANI_WALK_LEFT, true); // Walk Left
		m_Object->SetLookDirection(characterDir, XMFLOAT3(0.0f, 1.0f, 0.0f));
		m_AManager->UpdateAniPosition(0.0f, m_Object);
	}
	// W + D + Shift -> Run Right Up
		...
		if (!(m_PrevKeyBuffer['W'] & 0x80) || !(m_PrevKeyBuffer['D'] & 0x80) || !(m_PrevKeyBuffer[VK_LSHIFT] & 0x80)) {
			m_AManager->ChangeAnimation(ANI_RUN_RIGHT_UP, true); // Run Right Up
			m_Object->SetLookDirection(characterDir, XMFLOAT3(0.0f, 1.0f, 0.0f));
			m_AManager->UpdateAniPosition(0.0f, m_Object);
		}
		else {
			m_AManager->ChangeAnimation(ANI_RUN_RIGHT_UP, true); // Maintain Run
		}
	}
	// W + D + Shift -> Walk Right Up
	else if ((keyBuffer['W'] & 0x80) && (keyBuffer['D'] & 0x80) && (m_PrevKeyBuffer[VK_LSHIFT] & 0x80) && !(keyBuffer[VK_LSHIFT] & 0x80)) {
		m_AManager->ChangeAnimation(ANI_WALK_RIGHT_UP, true); // Walk Right Up
		m_Object->SetLookDirection(characterDir, XMFLOAT3(0.0f, 1.0f, 0.0f));
		m_AManager->UpdateAniPosition(0.0f, m_Object);
	}
	// W + D -> Walk Right Up
		...
		if (!(m_PrevKeyBuffer['W'] & 0x80) || !(m_PrevKeyBuffer['D'] & 0x80)) {
			...
			m_AManager->ChangeAnimation(ANI_WALK_RIGHT_UP, true); // Maintain Walk
		}
	}
	...
	// W + D, W -> Walk Right
		...
		m_AManager->ChangeAnimation(ANI_WALK_RIGHT, true); // Walk Right
		m_Object->SetLookDirection(characterDir, XMFLOAT3(0.0f, 1.0f, 0.0f));
		m_AManager->UpdateAniPosition(0.0f, m_Object);
	}
	// S + A + Shift -> Run Left Down
		...
		if (!(m_PrevKeyBuffer['S'] & 0x80) || !(m_PrevKeyBuffer['A'] & 0x80) || !(m_PrevKeyBuffer[VK_LSHIFT] & 0x80)) {
			m_AManager->ChangeAnimation(ANI_RUN_LEFT_DOWN, true); // Run Left Down
			m_Object->SetLookDirection(characterDir, XMFLOAT3(0.0f, 1.0f, 0.0f));
			m_AManager->UpdateAniPosition(0.0f, m_Object);
		}
		else {
			m_AManager->ChangeAnimation(ANI_RUN_LEFT_DOWN, true); // Maintain Run
		}
	}
	// S + A + Shift -> Walk Left Down
		...
		m_AManager->ChangeAnimation(ANI_WALK_LEFT_DOWN, true); // Walk Left Down
		m_Object->SetLookDirection(characterDir, XMFLOAT3(0.0f, 1.0f, 0.0f));
		m_AManager->UpdateAniPosition(0.0f, m_Object);
	}
	// S + A -> Walk Left Down
		...
		if (!(m_PrevKeyBuffer['S'] & 0x80) || !(m_PrevKeyBuffer['A'] & 0x80)) {
			...
			m_AManager->ChangeAnimation(ANI_WALK_LEFT_DOWN, true); // Maintain Walk
		}
	}
	// S + A + Shift, A -> Run Backward
	else if ((keyBuffer['S'] & 0x80) && (keyBuffer[VK_LSHIFT] & 0x80) && (m_PrevKeyBuffer['A'] & 0x80) && !(keyBuffer['A'] & 0x80)) {
		m_AManager->ChangeAnimation(ANI_RUN_BACKWARD, true); // Run Backward
		m_Object->SetLookDirection(characterDir, XMFLOAT3(0.0f, 1.0f, 0.0f));
		m_AManager->UpdateAniPosition(0.0f, m_Object);
	}
	// S + A, A -> Walk Backward
	else if ((keyBuffer['S'] & 0x80) && (m_PrevKeyBuffer['A'] & 0x80) && !(keyBuffer['A'] & 0x80)) {
		m_AManager->ChangeAnimation(ANI_WALK_BACKWARD, true); // Walk Backward
		m_Object->SetLookDirection(characterDir, XMFLOAT3(0.0f, 1.0f, 0.0f));
		m_AManager->UpdateAniPosition(0.0f, m_Object);
	}
	// S + A, S -> Walk Left
	...
	// S + D + Shift -> Run Right Down
		...
		if (!(m_PrevKeyBuffer['S'] & 0x80) || !(m_PrevKeyBuffer['D'] & 0x80) || !(m_PrevKeyBuffer[VK_LSHIFT] & 0x80)) {
			m_AManager->ChangeAnimation(ANI_RUN_RIGHT_DOWN, true); // Run Right Down
			m_Object->SetLookDirection(characterDir, XMFLOAT3(0.0f, 1.0f, 0.0f));
			m_AManager->UpdateAniPosition(0.0f, m_Object);
		}
		else {
			m_AManager->ChangeAnimation(ANI_RUN_RIGHT_DOWN, true); // Maintain Run
		}
	}
	// S + D + Shift -> Walk Right Down
		...
		m_AManager->ChangeAnimation(ANI_WALK_RIGHT_DOWN, true); // Walk Right Down
		m_Object->SetLookDirection(characterDir, XMFLOAT3(0.0f, 1.0f, 0.0f));
		m_AManager->UpdateAniPosition(0.0f, m_Object);
	}
	// S + D -> Walk Right Down
		...
		if (!(m_PrevKeyBuffer['S'] & 0x80) || !(m_PrevKeyBuffer['D'] & 0x80)) {
			...
			m_AManager->ChangeAnimation(ANI_WALK_RIGHT_DOWN, true); // Maintain Walk
		}
	}
	...
	// W + Shift -> Run Forward
		...
		if (!(m_PrevKeyBuffer['W'] & 0x80) || !(m_PrevKeyBuffer[VK_LSHIFT] & 0x80)) {
			...
			m_AManager->ChangeAnimation(ANI_RUN_FORWARD, true); // Maintain Run
		}
	}
	...
	// S + Shift -> Run Backward
		...
		if (!(m_PrevKeyBuffer['S'] & 0x80) || !(m_PrevKeyBuffer[VK_LSHIFT] & 0x80)) {
			...
			m_AManager->ChangeAnimation(ANI_RUN_BACKWARD, true); // Maintain Run
		}
	}
	...
	// S -> Walk Backward
		...
		if (!(m_PrevKeyBuffer['S'] & 0x80)) {
			...
			m_AManager->ChangeAnimation(ANI_WALK_BACKWARD, true); // Maintain Walk
		}
	}
	// A + Shift -> Run Left
		...
		if (!(m_PrevKeyBuffer['A'] & 0x80) || !(m_PrevKeyBuffer[VK_LSHIFT] & 0x80)) {
			m_AManager->ChangeAnimation(ANI_RUN_LEFT, true); // Run Left
			m_Object->SetLookDirection(characterDir, XMFLOAT3(0.0f, 1.0f, 0.0f));
			m_AManager->UpdateAniPosition(0.0f, m_Object);
		}
		else {
			m_AManager->ChangeAnimation(ANI_RUN_LEFT, true); // Maintain Run
		}
	}
	// A + Shift -> Walk Left
	...
	// A -> Walk Left
		...
		if (!(m_PrevKeyBuffer['A'] & 0x80)) {
			...
			m_AManager->ChangeAnimation(ANI_WALK_LEFT, true); // Maintain Walk
		}
	}
	// D + Shift -> Run Right
		...
		if (!(m_PrevKeyBuffer['D'] & 0x80) || !(m_PrevKeyBuffer[VK_LSHIFT] & 0x80)) {
			m_AManager->ChangeAnimation(ANI_RUN_RIGHT, true); // Run Right
			m_Object->SetLookDirection(characterDir, XMFLOAT3(0.0f, 1.0f, 0.0f));
			m_AManager->UpdateAniPosition(0.0f, m_Object);
		}
		else {
			m_AManager->ChangeAnimation(ANI_RUN_RIGHT, true); // Maintain Run
		}
	}
	// D + Shift -> Walk Right
	...
	// D -> Walk Right
		...
		if (!(m_PrevKeyBuffer['D'] & 0x80)) {
			...
			m_AManager->ChangeAnimation(ANI_WALK_RIGHT, true); // Maintain Walk
		}
	}
	// W -> IDLE
