...
void CRaytracingTestScene::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessage, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		switch (wParam) {
		case '1':
			m_pResourceManager->getAnimationManagers()[1]->setCurrnetSet(0);
			m_pResourceManager->getAnimationManagers()[1]->setTimeZero();
			break;
		case '2':
			...
			break;
		case '3':
			m_pResourceManager->getAnimationManagers()[1]->setCurrnetSet(2);
			m_pResourceManager->getAnimationManagers()[1]->setTimeZero();
			break;
		}
		break;
