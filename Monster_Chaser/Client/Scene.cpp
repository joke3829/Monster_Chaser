...
void CRaytracingScene::UpdateObject(float fElapsedTime)
{
	...
	for (auto& animationManager : animationManagers) {
		...
		if (!animationManager->IsInCombo() && animationManager->IsAnimationFinished()) {
			animationManager->ChangeAnimation(0, false); // idleÔøΩÔøΩ ÔøΩÔøΩ»Ø
			test = true;
	...
	if (test) {
		m_pResourceManager->UpdatePosition(fElapsedTime); //ÔøΩÔøΩƒ° ÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩ∆Æ
	}
...
void CRaytracingTestScene::MouseProcessing(HWND hWnd, UINT nMessage, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSEMOVE:
	{
		...
		if (oldPos.x != 0 || oldPos.y != 0) {
			...
			m_pCamera->Rotate(deltaX * 3.0f * 0.5f, 0.0f); // ƒ´∏ﬁ∂Û »∏¿¸ø° /3.0f «ÿ¡÷¥œ±Ó ±◊≥… ∞ˆ«ÿº≠ ƒ≥∏Ø≈Õ »∏¿¸¿Ã∂˚ ∞™¿ª ∏¬√·¥Ÿ.
...
void CRaytracingTestScene::ProcessInput(float fElapsedTime)
{
	...
	if ((keyBuffer['W'] & 0x80) && (keyBuffer['A'] & 0x80) && (keyBuffer[VK_LSHIFT] & 0x80)) {
		if (!(m_PrevKeyBuffer['W'] & 0x80) || !(m_PrevKeyBuffer['A'] & 0x80) || !(m_PrevKeyBuffer[VK_LSHIFT] & 0x80)) {
			m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(RUN_LEFT_UP, true); // ∂Ÿ±‚: øﬁ¬  ¥Î∞¢º± ¿ß
			m_pResourceManager->UpdatePosition(fElapsedTime);
		}
		else {
			m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(RUN_LEFT_UP, true); // ∂Ÿ±‚ ¿Ø¡ˆ
		}
	}
	
	// W + A + Shift ∂æ º¯∞£ (øﬁ¬  ¥Î∞¢º± ¿ß ∞»±‚∑Œ ¿¸»Ø)
	else if ((keyBuffer['W'] & 0x80) && (keyBuffer['A'] & 0x80) && (m_PrevKeyBuffer[VK_LSHIFT] & 0x80) && !(keyBuffer[VK_LSHIFT] & 0x80)) {
		m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(WALK_LEFT_UP, true); // ∞»±‚: øﬁ¬  ¥Î∞¢º± ¿ß
		m_pResourceManager->UpdatePosition(fElapsedTime);
	}
	// W + A ¥‹µ∂ (øﬁ¬  ¥Î∞¢º± ¿ß ∞»±‚)
		...
		if (!(m_PrevKeyBuffer['W'] & 0x80) || !(m_PrevKeyBuffer['A'] & 0x80)) {
			...
			m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(WALK_LEFT_UP, true); // ∞»±‚ ¿Ø¡ˆ
		}
	}
	// W + A + Shiftø°º≠ A ∂æ º¯∞£ (¿¸¡¯ ∂Ÿ±‚∑Œ ¿¸»Ø)
	else if ((keyBuffer['W'] & 0x80) && (keyBuffer[VK_LSHIFT] & 0x80) && (m_PrevKeyBuffer['A'] & 0x80) && !(keyBuffer['A'] & 0x80)) {
		m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(RUN_FORWARD, true); // ∂Ÿ±‚: ¿¸¡¯
		m_pResourceManager->UpdatePosition(fElapsedTime);
	}
	// W + Aø°º≠ A ∂æ º¯∞£ (¿¸¡¯ ∞»±‚∑Œ ¿¸»Ø)
	else if ((keyBuffer['W'] & 0x80) && (m_PrevKeyBuffer['A'] & 0x80) && !(keyBuffer['A'] & 0x80)) {
		m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(WALK_FORWARD, true); // ∞»±‚: ¿¸¡¯
		m_pResourceManager->UpdatePosition(fElapsedTime);
	}
	// W + Aø°º≠ W ∂æ º¯∞£ (¡¬√¯ ∞»±‚∑Œ ¿¸»Ø)
	else if ((keyBuffer['A'] & 0x80) && (m_PrevKeyBuffer['W'] & 0x80) && !(keyBuffer['W'] & 0x80)) {
		m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(WALK_LEFT, true); // ∞»±‚: ¡¬√¯ (∞°¡§: 8)
		m_pResourceManager->UpdatePosition(fElapsedTime);
	}
	// W + D + Shift (ø¿∏•¬  ¥Î∞¢º± ¿ß ∂Ÿ±‚)
		...
		if (!(m_PrevKeyBuffer['W'] & 0x80) || !(m_PrevKeyBuffer['D'] & 0x80) || !(m_PrevKeyBuffer[VK_LSHIFT] & 0x80)) {
			m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(RUN_RIGHT_UP, true); // ∂Ÿ±‚: ø¿∏•¬  ¥Î∞¢º± ¿ß
			m_pResourceManager->UpdatePosition(fElapsedTime);
		}
		else {
			m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(RUN_RIGHT_UP, true); // ∂Ÿ±‚ ¿Ø¡ˆ
		}
	}
	// W + D + Shift ∂æ º¯∞£ (ø¿∏•¬  ¥Î∞¢º± ¿ß ∞»±‚∑Œ ¿¸»Ø)
	else if ((keyBuffer['W'] & 0x80) && (keyBuffer['D'] & 0x80) && (m_PrevKeyBuffer[VK_LSHIFT] & 0x80) && !(keyBuffer[VK_LSHIFT] & 0x80)) {
		m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(WALK_RIGHT_UP, true); // ∞»±‚: ø¿∏•¬  ¥Î∞¢º± ¿ß
		m_pResourceManager->UpdatePosition(fElapsedTime);
	}
	// W + D ¥‹µ∂ (ø¿∏•¬  ¥Î∞¢º± ¿ß ∞»±‚)
		...
		if (!(m_PrevKeyBuffer['W'] & 0x80) || !(m_PrevKeyBuffer['D'] & 0x80)) {
			...
			m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(WALK_RIGHT_UP, true); // ∞»±‚ ¿Ø¡ˆ
		}
	}
	// W + D + Shiftø°º≠ D ∂æ º¯∞£ (¿¸¡¯ ∂Ÿ±‚∑Œ ¿¸»Ø)
	else if ((keyBuffer['W'] & 0x80) && (keyBuffer[VK_LSHIFT] & 0x80) && (m_PrevKeyBuffer['D'] & 0x80) && !(keyBuffer['D'] & 0x80)) {
		m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(RUN_FORWARD, true); // ÔøΩŸ±ÔøΩ: ÔøΩÔøΩÔøΩÔøΩ
		m_pResourceManager->UpdatePosition(fElapsedTime);
	}
	// W + DÔøΩÔøΩÔøΩÔøΩ D ÔøΩÔøΩ ÔøΩÔøΩÔøΩÔøΩ (ÔøΩÔøΩÔøΩÔøΩ ÔøΩ»±ÔøΩÔø?ÔøΩÔøΩ»Ø)
	else if ((keyBuffer['W'] & 0x80) && (m_PrevKeyBuffer['D'] & 0x80) && !(keyBuffer['D'] & 0x80)) {
		m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(WALK_FORWARD, true); // ÔøΩ»±ÔøΩ: ÔøΩÔøΩÔøΩÔøΩ
		m_pResourceManager->UpdatePosition(fElapsedTime);
	}
	// W + DÔøΩÔøΩÔøΩÔøΩ W ÔøΩÔøΩ ÔøΩÔøΩÔøΩÔøΩ (ÔøΩÔøΩÔøΩÔøΩ ÔøΩ»±ÔøΩÔø?ÔøΩÔøΩ»Ø)
	else if ((keyBuffer['D'] & 0x80) && (m_PrevKeyBuffer['W'] & 0x80) && !(keyBuffer['W'] & 0x80)) {
		m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(WALK_RIGHT, true); // ÔøΩ»±ÔøΩ: ÔøΩÔøΩÔøΩÔøΩ (ÔøΩÔøΩÔøΩÔøΩ: 9)
		m_pResourceManager->UpdatePosition(fElapsedTime);
	}
	// S + A + Shift (ÔøΩÔøΩÔøΩÔøΩ ÔøΩÎ∞¢ÔøΩÔøΩ ÔøΩÔøΩ ÔøΩŸ±ÔøΩ)
		...
		if (!(m_PrevKeyBuffer['S'] & 0x80) || !(m_PrevKeyBuffer['A'] & 0x80) || !(m_PrevKeyBuffer[VK_LSHIFT] & 0x80)) {
			m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(RUN_LEFT_DOWN, true); // ÔøΩŸ±ÔøΩ: ÔøΩÔøΩ ÔøΩÔøΩ ÔøΩÎ∞¢ÔøΩÔøΩ ÔøΩÔøΩ
			m_pResourceManager->UpdatePosition(fElapsedTime);
		}
		else {
			m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(RUN_LEFT_DOWN, true); // ÔøΩŸ±ÔøΩ ÔøΩÔøΩÔøΩÔøΩ
		}
	}
	// S + A + Shift ÔøΩÔøΩ ÔøΩÔøΩÔøΩÔøΩ (ÔøΩÔøΩÔøΩÔøΩ ÔøΩÎ∞¢ÔøΩÔøΩ ÔøΩÔøΩ ÔøΩ»±ÔøΩÔø?ÔøΩÔøΩ»Ø)
	else if ((keyBuffer['S'] & 0x80) && (keyBuffer['A'] & 0x80) && (m_PrevKeyBuffer[VK_LSHIFT] & 0x80) && !(keyBuffer[VK_LSHIFT] & 0x80)) {
		m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(WALK_LEFT_DOWN, true); // ÔøΩ»±ÔøΩ: ÔøΩÔøΩÔøΩÔøΩ ÔøΩÎ∞¢ÔøΩÔøΩ ÔøΩÔøΩ
		m_pResourceManager->UpdatePosition(fElapsedTime);
	}
	// S + A ÔøΩ‹µÔøΩ (ÔøΩÔøΩÔøΩÔøΩ ÔøΩÎ∞¢ÔøΩÔøΩ ÔøΩÔøΩ ÔøΩ»±ÔøΩ)
		...
		if (!(m_PrevKeyBuffer['S'] & 0x80) || !(m_PrevKeyBuffer['A'] & 0x80)) {
			...
			m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(WALK_LEFT_DOWN, true); // ÔøΩ»±ÔøΩ ÔøΩÔøΩÔøΩÔøΩ
		}
	}
	// S + A + ShiftÔøΩÔøΩÔøΩÔøΩ A ÔøΩÔøΩ ÔøΩÔøΩÔøΩÔøΩ (ÔøΩÔøΩÔøΩÔøΩ ÔøΩŸ±ÔøΩÔø?ÔøΩÔøΩ»Ø)
	else if ((keyBuffer['S'] & 0x80) && (keyBuffer[VK_LSHIFT] & 0x80) && (m_PrevKeyBuffer['A'] & 0x80) && !(keyBuffer['A'] & 0x80)) {
		m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(RUN_BACKWARD, true); // ÔøΩŸ±ÔøΩ: ÔøΩÔøΩÔøΩÔøΩ (ÔøΩÔøΩÔøΩÔøΩ: 20)
		m_pResourceManager->UpdatePosition(fElapsedTime);
	}
	// S + AÔøΩÔøΩÔøΩÔøΩ A ÔøΩÔøΩ ÔøΩÔøΩÔøΩÔøΩ (ÔøΩÔøΩÔøΩÔøΩ ÔøΩ»±ÔøΩÔø?ÔøΩÔøΩ»Ø)
	else if ((keyBuffer['S'] & 0x80) && (m_PrevKeyBuffer['A'] & 0x80) && !(keyBuffer['A'] & 0x80)) {
		m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(WALK_BACKWARD, true); // ÔøΩ»±ÔøΩ: ÔøΩÔøΩÔøΩÔøΩ
		m_pResourceManager->UpdatePosition(fElapsedTime);
	}
	// S + AÔøΩÔøΩÔøΩÔøΩ S ÔøΩÔøΩ ÔøΩÔøΩÔøΩÔøΩ (ÔøΩÔøΩÔøΩÔøΩ ÔøΩ»±ÔøΩÔø?ÔøΩÔøΩ»Ø)
	else if ((keyBuffer['A'] & 0x80) && (m_PrevKeyBuffer['S'] & 0x80) && !(keyBuffer['S'] & 0x80)) {
		m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(WALK_LEFT, true); // ÔøΩ»±ÔøΩ: ÔøΩÔøΩÔøΩÔøΩ (ÔøΩÔøΩÔøΩÔøΩ: 8)
		m_pResourceManager->UpdatePosition(fElapsedTime);
	}
	// S + D + Shift (ÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩ ÔøΩÎ∞¢ÔøΩÔøΩ ÔøΩÔøΩ ÔøΩŸ±ÔøΩ)
		...
		if (!(m_PrevKeyBuffer['S'] & 0x80) || !(m_PrevKeyBuffer['D'] & 0x80) || !(m_PrevKeyBuffer[VK_LSHIFT] & 0x80)) {
			m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(RUN_RIGHT_DOWN, true); // ÔøΩŸ±ÔøΩ: ÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩ ÔøΩÎ∞¢ÔøΩÔøΩ ÔøΩÔøΩ
			m_pResourceManager->UpdatePosition(fElapsedTime);
		}
		else {
			m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(RUN_RIGHT_DOWN, true); // ÔøΩŸ±ÔøΩ ÔøΩÔøΩÔøΩÔøΩ
		}
	}
	// S + D + Shift ÔøΩÔøΩ ÔøΩÔøΩÔøΩÔøΩ (ÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩ ÔøΩÎ∞¢ÔøΩÔøΩ ÔøΩÔøΩ ÔøΩ»±ÔøΩÔø?ÔøΩÔøΩ»Ø)
	else if ((keyBuffer['S'] & 0x80) && (keyBuffer['D'] & 0x80) && (m_PrevKeyBuffer[VK_LSHIFT] & 0x80) && !(keyBuffer[VK_LSHIFT] & 0x80)) {
		m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(WALK_RIGHT_DOWN, true); // ÔøΩ»±ÔøΩ: ÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩ ÔøΩÎ∞¢ÔøΩÔøΩ ÔøΩÔøΩ
		m_pResourceManager->UpdatePosition(fElapsedTime);
	}
	// S + D ÔøΩ‹µÔøΩ (ÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩ ÔøΩÎ∞¢ÔøΩÔøΩ ÔøΩÔøΩ ÔøΩ»±ÔøΩ)
		...
		if (!(m_PrevKeyBuffer['S'] & 0x80) || !(m_PrevKeyBuffer['D'] & 0x80)) {
			...
			m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(WALK_RIGHT_DOWN, true); // ÔøΩ»±ÔøΩ ÔøΩÔøΩÔøΩÔøΩ
		}
	}
	// S + D + ShiftÔøΩÔøΩÔøΩÔøΩ D ÔøΩÔøΩ ÔøΩÔøΩÔøΩÔøΩ (ÔøΩÔøΩÔøΩÔøΩ ÔøΩŸ±ÔøΩÔø?ÔøΩÔøΩ»Ø)
	else if ((keyBuffer['S'] & 0x80) && (keyBuffer[VK_LSHIFT] & 0x80) && (m_PrevKeyBuffer['D'] & 0x80) && !(keyBuffer['D'] & 0x80)) {
		m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(RUN_BACKWARD, true); // ÔøΩŸ±ÔøΩ: ÔøΩÔøΩÔøΩÔøΩ (ÔøΩÔøΩÔøΩÔøΩ: 20)
		m_pResourceManager->UpdatePosition(fElapsedTime);
	}
	// S + DÔøΩÔøΩÔøΩÔøΩ D ÔøΩÔøΩ ÔøΩÔøΩÔøΩÔøΩ (ÔøΩÔøΩÔøΩÔøΩ ÔøΩ»±ÔøΩÔø?ÔøΩÔøΩ»Ø)
	else if ((keyBuffer['S'] & 0x80) && (m_PrevKeyBuffer['D'] & 0x80) && !(keyBuffer['D'] & 0x80)) {
		m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(WALK_BACKWARD, true); // ÔøΩ»±ÔøΩ: ÔøΩÔøΩÔøΩÔøΩ
		m_pResourceManager->UpdatePosition(fElapsedTime);
	}
	// S + DÔøΩÔøΩÔøΩÔøΩ S ÔøΩÔøΩ ÔøΩÔøΩÔøΩÔøΩ (ÔøΩÔøΩÔøΩÔøΩ ÔøΩ»±ÔøΩÔø?ÔøΩÔøΩ»Ø)
	else if ((keyBuffer['D'] & 0x80) && (m_PrevKeyBuffer['S'] & 0x80) && !(keyBuffer['S'] & 0x80)) {
		m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(WALK_RIGHT, true); // ÔøΩ»±ÔøΩ: ÔøΩÔøΩÔøΩÔøΩ (ÔøΩÔøΩÔøΩÔøΩ: 9)
		m_pResourceManager->UpdatePosition(fElapsedTime);
	}
	// W + Shift (ÔøΩÔøΩÔøΩÔøΩ ÔøΩŸ±ÔøΩ)
	...
	// W + Shift ÔøΩÔøΩ ÔøΩÔøΩÔøΩÔøΩ (ÔøΩÔøΩÔøΩÔøΩ ÔøΩ»±ÔøΩÔø?ÔøΩÔøΩ»Ø)
	else if ((keyBuffer['W'] & 0x80) && (m_PrevKeyBuffer[VK_LSHIFT] & 0x80) && !(keyBuffer[VK_LSHIFT] & 0x80)) {
		m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(WALK_FORWARD, true); // ÔøΩ»±ÔøΩ: ÔøΩÔøΩÔøΩÔøΩ
		m_pResourceManager->UpdatePosition(fElapsedTime);
	}
	// W ÔøΩ‹µÔøΩ (ÔøΩÔøΩÔøΩÔøΩ ÔøΩ»±ÔøΩ)
	...
	// S + Shift (ÔøΩÔøΩÔøΩÔøΩ ÔøΩŸ±ÔøΩ)
		...
		if (!(m_PrevKeyBuffer['S'] & 0x80) || !(m_PrevKeyBuffer[VK_LSHIFT] & 0x80)) {
			...
			m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(RUN_BACKWARD, true); // ÔøΩŸ±ÔøΩ ÔøΩÔøΩÔøΩÔøΩ
		}
	}
	// S + Shift ÔøΩÔøΩ ÔøΩÔøΩÔøΩÔøΩ (ÔøΩÔøΩÔøΩÔøΩ ÔøΩ»±ÔøΩÔø?ÔøΩÔøΩ»Ø)
	else if ((keyBuffer['S'] & 0x80) && (m_PrevKeyBuffer[VK_LSHIFT] & 0x80) && !(keyBuffer[VK_LSHIFT] & 0x80)) {
		m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(WALK_BACKWARD, true); // ÔøΩ»±ÔøΩ: ÔøΩÔøΩÔøΩÔøΩ
		m_pResourceManager->UpdatePosition(fElapsedTime);
	}
	// S ÔøΩ‹µÔøΩ (ÔøΩÔøΩÔøΩÔøΩ ÔøΩ»±ÔøΩ)
	...
	// A + Shift (ÔøΩÔøΩÔøΩÔøΩ ÔøΩŸ±ÔøΩ)
	else if ((keyBuffer['A'] & 0x80) && (keyBuffer[VK_LSHIFT] & 0x80)) {
		if (!(m_PrevKeyBuffer['A'] & 0x80) || !(m_PrevKeyBuffer[VK_LSHIFT] & 0x80)) {
			m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(RUN_LEFT, true); // ÔøΩŸ±ÔøΩ: ÔøΩÔøΩÔøΩÔøΩ (ÔøΩÔøΩÔøΩÔøΩ: 18)
			m_pResourceManager->UpdatePosition(fElapsedTime);
		}
		else {
			m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(RUN_LEFT, true); // ÔøΩŸ±ÔøΩ ÔøΩÔøΩÔøΩÔøΩ
		}
	}
	// A + Shift ÔøΩÔøΩ ÔøΩÔøΩÔøΩÔøΩ (ÔøΩÔøΩÔøΩÔøΩ ÔøΩ»±ÔøΩÔø?ÔøΩÔøΩ»Ø)
	else if ((keyBuffer['A'] & 0x80) && (m_PrevKeyBuffer[VK_LSHIFT] & 0x80) && !(keyBuffer[VK_LSHIFT] & 0x80)) {
		m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(WALK_LEFT, true); // ÔøΩ»±ÔøΩ: ÔøΩÔøΩÔøΩÔøΩ (ÔøΩÔøΩÔøΩÔøΩ: 8)
		m_pResourceManager->UpdatePosition(fElapsedTime);
	}
	// A ÔøΩ‹µÔøΩ (ÔøΩÔøΩÔøΩÔøΩ ÔøΩ»±ÔøΩ)
		...
		if (!(m_PrevKeyBuffer['A'] & 0x80)) {
			...
			m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(WALK_LEFT, true); // ÔøΩ»±ÔøΩ ÔøΩÔøΩÔøΩÔøΩ
		}
	}
	// D + Shift (ÔøΩÔøΩÔøΩÔøΩ ÔøΩŸ±ÔøΩ)
	else if ((keyBuffer['D'] & 0x80) && (keyBuffer[VK_LSHIFT] & 0x80)) {
		if (!(m_PrevKeyBuffer['D'] & 0x80) || !(m_PrevKeyBuffer[VK_LSHIFT] & 0x80)) {
			m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(RUN_RIGHT, true); // ÔøΩŸ±ÔøΩ: ÔøΩÔøΩÔøΩÔøΩ (ÔøΩÔøΩÔøΩÔøΩ: 19)
			m_pResourceManager->UpdatePosition(fElapsedTime);
		}
		else {
			m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(RUN_RIGHT, true); // ÔøΩŸ±ÔøΩ ÔøΩÔøΩÔøΩÔøΩ
		}
	}
	// D + Shift ÔøΩÔøΩ ÔøΩÔøΩÔøΩÔøΩ (ÔøΩÔøΩÔøΩÔøΩ ÔøΩ»±ÔøΩÔø?ÔøΩÔøΩ»Ø)
	else if ((keyBuffer['D'] & 0x80) && (m_PrevKeyBuffer[VK_LSHIFT] & 0x80) && !(keyBuffer[VK_LSHIFT] & 0x80)) {
		m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(WALK_RIGHT, true); // ÔøΩ»±ÔøΩ: ÔøΩÔøΩÔøΩÔøΩ (ÔøΩÔøΩÔøΩÔøΩ: 9)
		m_pResourceManager->UpdatePosition(fElapsedTime);
	}
	// D ÔøΩ‹µÔøΩ (ÔøΩÔøΩÔøΩÔøΩ ÔøΩ»±ÔøΩ)
		...
		if (!(m_PrevKeyBuffer['D'] & 0x80)) {
			...
			m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(WALK_RIGHT, true); // ÔøΩ»±ÔøΩ ÔøΩÔøΩÔøΩÔøΩ
		}
	}
	// W ÔøΩÔøΩ ÔøΩÔøΩÔøΩÔøΩ (IDLEÔøΩÔøΩ ÔøΩÔøΩ»Ø)
	...
	// S ÔøΩÔøΩ ÔøΩÔøΩÔøΩÔøΩ (IDLEÔøΩÔøΩ ÔøΩÔøΩ»Ø)
	...
	// A ÔøΩÔøΩ ÔøΩÔøΩÔøΩÔøΩ (IDLEÔøΩÔøΩ ÔøΩÔøΩ»Ø)
	...
	// D ÔøΩÔøΩ ÔøΩÔøΩÔøΩÔøΩ (IDLEÔøΩÔøΩ ÔøΩÔøΩ»Ø)
	...
	if (keyBuffer['H'] & 0x80) {
		m_pResourceManager->getSkinningObjectList()[Client.get_id()]->Rotate(XMFLOAT3(0.0f, 180.0f * fElapsedTime, 0.0f)); //ÔøΩÔøΩ»∏ÔøΩÔøΩ
	}
	...
	if ((keyBuffer['J'] & 0x80) && !(m_PrevKeyBuffer['J'] & 0x80)) {
		m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(1, true); //ÔøΩÔøΩÔøΩœ∞ÔøΩ ÔøΩ¬±ÔøΩ
		m_LockAnimation1 = true;
	}

	if ((keyBuffer['K'] & 0x80) && !(m_PrevKeyBuffer['K'] & 0x80)) {
		m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(2, true); //ÔøΩÔøΩÔøΩœ∞ÔøΩ ÔøΩ¬∞ÔøΩ ÔøΩ◊±ÔøΩ
		m_LockAnimation1 = true;
	}
	...
	if ((keyBuffer['L'] & 0x80) && !(m_PrevKeyBuffer['L'] & 0x80)) {
		m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(3, true); //ÔøΩÔøΩÔøΩœ∞ÔøΩ ÔøΩ¬±ÔøΩ
		m_LockAnimation1 = true;
	}

	if ((keyBuffer['U'] & 0x80) && !(m_PrevKeyBuffer['U'] & 0x80)) {
		m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(4, true); //ÔøΩÔøΩÔøΩœ∞ÔøΩ ÔøΩ¬∞ÔøΩ ÔøΩ◊±ÔøΩ
		m_LockAnimation1 = true;
	}

	if ((keyBuffer['2'] & 0x80) && !(m_PrevKeyBuffer['2'] & 0x80)) {
		m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(31, true); //ÔøΩÔøΩ≈≥2
		m_LockAnimation1 = true;
	}

	if ((keyBuffer['1'] & 0x80) && !(m_PrevKeyBuffer['1'] & 0x80)) {
		m_pResourceManager->getAnimationManagers()[Client.get_id()]->ChangeAnimation(32, true); //ÔøΩÔøΩ≈≥1
		m_LockAnimation1 = true;
	}

	if ((keyBuffer['3'] & 0x80) && !(m_PrevKeyBuffer['3'] & 0x80)) {
		m_pResourceManager->getAnimationManagers()[Client.get_id()]->StartSkill3(); //ÔøΩÔøΩ≈≥3
		m_LockAnimation = true;
	}
	// ÔøΩÔøΩÔøΩÔøΩ ≈∞ ÔøΩÔøΩÔøΩ¬∏ÔøΩ ÔøΩÔøΩÔøΩÔøΩ ÔøΩÔøΩÔøΩ¬∑ÔøΩ ÔøΩÔøΩÔøΩÔøΩ
